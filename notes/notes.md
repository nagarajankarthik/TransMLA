...
Let $q_n = \sum_{i = 1}^{gd} q_i^2$ and $k_n = \sum_{i = 1}^{gd} k_i^2$. The dot product becomes
...
The different weights used for the various channels means that the RMSNorm operation must be applied after the initial projection of hidden states to obtain the query vectors and before the rotation is applied. This is required to ensure that the magnitude of the dot product is preserved after the inclusion of the rotation operation. However, this prevents the operation for up-projection of keys from being absorbed into the calculation of the final query vector that is used for the dot product with the key vector. As explained on page 5 of the [TransMLA paper](https://arxiv.org/pdf/2502.07864), the absorb operation is critical for maximizing inference throughput.
